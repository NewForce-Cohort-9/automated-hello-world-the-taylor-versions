export const italyCities = [
  {
    name: "Rome",
    image:
      "https://www.italyperfect.com/cdn-cgi/image/format=auto,width=1256/https://www.italyperfect.com/g/photos/upload/sml_845543004-1590582528-ip-info-rome.jpg",
    description:
      "Rome is the capital city of Italy. It is also the capital of the Lazio region, the centre of the Metropolitan City of Rome Capital, and a special comune (municipality) named Comune di Roma Capitale.",
  },
  {
    name: "Milan",
    image:
      "https://i.natgeofe.com/n/485b2f58-8e2b-4633-a1c5-658bf513e53e/alps-milan-italy_16x9.jpg",
    description:
      "Milan sits right at the heart of Italy's fashion, design, and architecture scenes—so much so, in fact, that creatives from all over the world come every year for events like Salone del Mobile (the Milan Furniture Fair) and Milan Fashion Week. And the scenery and sights truly live up to the hype",
  },
  {
    name: "Sicily",
    image:
      "https://www.worldatlas.com/upload/a1/74/06/cefalu-town-sicily-italy-andrew-mayovskyy.jpg",
    description:
      "Sicily is unlike anywhere else in Italy. Sure, the Mediterranean island offers everything that makes Italy a major destination: beaches, wine, food, and architecture—but all with a Sicilian twist. Rustic fare makes the most of local produce from rich cannoli to crispy creamy arancini.",
  },
];
